...
export default function StrategyCard({
  ...
  return (
    <motion.div
      ...
      className="group relative rounded-[18px] border border-foreground/[0.04] bg-foreground/[0.012] p-5 hover:border-foreground/[0.08] transition-all duration-500 cursor-pointer overflow-hidden h-full"
    >
      <div
      ...
      <div className="relative z-10">
        {badge && (
          <span className="absolute top-0 right-0 rounded-[8px] bg-foreground/[0.04] px-2 py-[2px] text-[9px] font-semibold text-foreground/25 border border-foreground/[0.03]">
            {badge}
          </span>
        )}
        ...
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-[13px] font-semibold text-foreground/75 tracking-[-0.01em]">
            {title}
          </h3>
        </div>
        <p className="text-[10px] text-foreground/15 mb-4 font-medium pl-3">{market}</p>

        <div className="flex items-center gap-5 mb-4">
          {stats.map((stat) => (
            <div key={stat.label}>
              <p className="text-[14px] font-bold text-foreground/60 tabular-nums tracking-tight">
                {stat.value}
              </p>
              <p className="text-[8px] text-foreground/15 uppercase tracking-[0.1em] font-semibold mt-0.5">
                {stat.label}
              </p>
            </div>
          ))}
        ...
        <div className="flex items-center justify-between pt-3 border-t border-foreground/[0.03]">
          <span className="text-[10px] text-foreground/10 tabular-nums font-medium">
            {runs} runs
          </span>
          <span className="text-[10px] text-foreground/15 group-hover:text-foreground/40 transition-colors duration-500 font-semibold tracking-wide">
            VIEW &rarr;
          </span>
        </div>
      </div>

...
function parseAssistantResponse(content: string) {
  ...
  // Clean display content (remove markers)
  ...
  try {
    ...
    if (anthropic) {
      try {
        ...
        assistantContent = textBlock?.text || 'Sorry, I could not generate a response.'
      } catch (apiError) {
        console.error('Claude API error, falling back to mock:', apiError instanceof Error ? apiError.message : apiError)
        assistantContent = generateMockResponse(message)
      }
    } else {
      // Mock response when no Anthropic key is set
      assistantContent = generateMockResponse(message)
    }
